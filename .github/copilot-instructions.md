...
- Scope discovery to the configured root page and its descendant `child_page` blocks. Do not search or export every page accessible to the integration.
...
- Document and test readable fallbacks for toggles, embeds, synced blocks, columns, databases, and unsupported blocks. Columns may flatten, and Markdown/HTML approximations are acceptable.
...
- `src/notion.ts`: root-scoped traversal and page metadata extraction.
...
Before calling a release ready, verify that nested pages export, new children appear automatically, a one-sentence edit changes only its page, a no-op run produces no Git changes, nested tasks are valid GFM, unusual and duplicate titles are safe, rename paths remain stable, orphan policy works, and no secret appears in logs.
